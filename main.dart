...
class _AuthScreenState extends State<AuthScreen> {
  ...
  Future<void> _authenticate() async {
    if (emailController.text.trim().isEmpty || passwordController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please enter Email and Password!'), backgroundColor: Colors.redAccent));
      return;
    }

    setState(() => isLoading = true);
    final url = isLogin 
        ? 'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${OnePlaceApp.apiKey}'
        : 'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=${OnePlaceApp.apiKey}';
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Connection failed! Check your internet.'), backgroundColor: Colors.redAccent));
    }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(isLogin ? 'One Place Login' : 'Create Account', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, letterSpacing: 1)),
              ...
              isLoading 
                ...
                : SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: Text(isLogin ? 'Login' : 'Register', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                    ),
                  ),
              const SizedBox(height: 10),
              TextButton(
                onPressed: () => setState(() => isLogin = !isLogin),
                child: Text(isLogin ? 'Create a new account?' : 'Already have an account? Login', style: const TextStyle(color: Color(0xFF6366F1), fontSize: 14)),
              )
            ],
          ),
        ),
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    super.initState();
    dbUrl = 'https://irsad-b0b2d-default-rtdb.asia-southeast1.firebasedatabase.app/users/${widget.uid}/one_place_entries.json?auth=${widget.token}';
    loadData();
  }
  ...
  Future<void> loadData() async {
    try {
      ...
      if (response.statusCode == 200 && response.body != 'null' && response.body != '{}') {
        ...
        final prefs = await SharedPreferences.getInstance();
        await prefs.setString('one_place_entries', jsonEncode(entries.map((e) => e.toJson()).toList()));
      } else {
        _loadLocalBackup();
      }
    } catch (e) {
  ...
  Future<void> _loadLocalBackup() async {
    final prefs = await SharedPreferences.getInstance();
    String? data = prefs.getString('one_place_entries');
    if (data != null) {
  ...
  Future<void> saveData() async {
    ...
    await prefs.setString('one_place_entries', encoded);
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Synced to Cloud! ✓'), backgroundColor: Colors.green, duration: Duration(seconds: 1)));
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Offline mode. Saved to phone.'), backgroundColor: Colors.orange, duration: Duration(seconds: 2)));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('One Place', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20)),
        actions: [
          IconButton(
...
class MemberDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('$memberName Details')),
      body: Column(
        children: [
          Container(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                const Text('Total Outstanding:', style: TextStyle(fontSize: 16, color: Colors.grey)),
          ...
          const Padding(
            ...
            child: Align(alignment: Alignment.centerLeft, child: Text('Transaction History', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold))),
          ),
          Expanded(
    ...
    return Column(
      ...
      children: [
        const Padding(
          ...
          child: Text('All Members Ledger', style: TextStyle(color: Colors.grey, fontSize: 14, fontWeight: FontWeight.bold)),
        ),
        Expanded(
          child: isLoading 
                                ...
                                children: [
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      ...
                                      const Text('View full ledger ->', style: TextStyle(fontSize: 12, color: Colors.grey)),
                                    ],
                                  ),
                                ],
                              ),
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        const Padding(
          padding: EdgeInsets.symmetric(horizontal: 16, vertical: 5),
          child: Text('All Transactions History', style: TextStyle(color: Colors.grey, fontSize: 14, fontWeight: FontWeight.bold)),
        ),
        Expanded(
          child: isLoading 
            ? const Center(child: CircularProgressIndicator())
    ...
    return Padding(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Add New Entry', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 20),
            if (existingNames.isNotEmpty) ...[
              DropdownButtonFormField<String>(
                dropdownColor: const Color(0xFF1E1E1E),
                decoration: InputDecoration(labelText: 'Select Existing Member', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))),
            ...
            TextField(controller: nameController, decoration: InputDecoration(labelText: 'Enter Name', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
            const SizedBox(height: 12),
            TextField(
              controller: TextEditingController(text: selectedDate), readOnly: true, decoration: InputDecoration(labelText: 'Date', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), suffixIcon: const Icon(Icons.calendar_today_rounded)),
              onTap: () async {
            ...
            const SizedBox(height: 12),
            TextField(controller: itemController, decoration: InputDecoration(labelText: 'Item / Description (e.g., Grocery, Cash)', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
            const SizedBox(height: 12),
            TextField(controller: priceController, keyboardType: TextInputType.number, decoration: InputDecoration(labelText: 'Amount (₹)', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), prefixIcon: const Icon(Icons.currency_rupee_rounded))),
                ...
                Expanded(
                  child: ElevatedButton(
                    ...
                    onPressed: () {
                      ...
                      Entry newEntry = Entry(id: DateTime.now().millisecondsSinceEpoch, name: nameController.text.trim(), date: selectedDate, item: itemController.text.trim().isEmpty ? 'General Entry' : itemController.text.trim(), price: price);
